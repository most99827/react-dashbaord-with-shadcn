...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<"div"> & { showIcon?: boolean }) {
  const [skeletonStyle] = React.useState(() => ({ "--skeleton-width": `${Math.floor(Math.random() * 40) + 50}%` } as React.CSSProperties))
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-sidebar-border ms-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-s px-2.5 py-0.5",
